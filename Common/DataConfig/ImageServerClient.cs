...
namespace ETC.Windows.Business.Layer4
{
    /// <summary>
    ...
    public class ImageServerClient
    {
        ...
        public ImageServerClient(String addr, int portNumber, String user, String password)
        {
            ...
            try
            {
                ...
                Provider or = new Provider();
                string strConfig = or.LoadInforConfigProxy();
                string[] config = strConfig.Split(';');
                if (config[4] == "true")
                {
                    m_ISSocket = SocksProxy.ConnectToSocks5Proxy(config[0], ushort.Parse(config[1]), addr, ushort.Parse(portNumber.ToString()), config[2], config[3]);
                }
                else
                {
